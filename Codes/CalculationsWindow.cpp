...
CalculationsWindow::CalculationsWindow(controller* crt,QWidget* parent)
    ...
    connect(backButton, &QPushButton::clicked, this, &CalculationsWindow::close);
}

void CalculationsWindow::setDrawer(QGraphicsScene* sharedScene){
...
void CalculationsWindow::setProcessTable() {
    if(!Process::processes.empty()){
        ...
        for (int i = 0; i < temp.size(); ++i) {
            ...
            if (isPriority) {
                ...
                if(p->getStartTime() == -1)
                    processTable->setItem(i, 8, new QTableWidgetItem(QString::number(-1)));
                else
                ...
                if(p->getStartTime() == -1)
                    processTable->setItem(i, 7, new QTableWidgetItem(QString::number(-1)));
                else
...
void CalculationsWindow::calculate(){
    std::vector<Process*> temp = Process::processes;
